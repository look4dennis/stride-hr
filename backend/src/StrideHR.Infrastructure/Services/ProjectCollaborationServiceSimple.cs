...
public class ProjectCollaborationServiceSimple : IProjectCollaborationService
{
    ...
    private readonly ILogger<ProjectCollaborationServiceSimple> _logger;

    public ProjectCollaborationServiceSimple(
        ...
        IMapper mapper,
        ILogger<ProjectCollaborationServiceSimple> logger)
    {
    ...
    public async Task<ProjectCommunicationStatsDto> GetProjectCommunicationStatsAsync(int projectId)
    {
        try
        {
            ...
            return new ProjectCommunicationStatsDto
            {
                ...
                ActiveTeamMembers = 0, // Would calculate from team members
                LastActivity = activities.OrderByDescending(a => a.CreatedAt).FirstOrDefault()?.CreatedAt ?? DateTime.MinValue,
                MemberActivities = new List<TeamMemberActivityDto>()
            };
        }
        catch (Exception ex)
        {
    ...
    public async Task NotifyTeamMembersAsync(int projectId, string message, string notificationType)
    {
        try
        {
            // Simplified implementation - would use SignalR in real implementation
            _logger.LogInformation("Notification sent to team members for project {ProjectId}: {Message}", projectId, message);
        }
        catch (Exception ex)
        {
    ...
    public async Task NotifyTaskAssigneesAsync(int taskId, string message, string notificationType)
    {
        ...
        {
            // Simplified implementation - would use SignalR in real implementation
            _logger.LogInformation("Notification sent to task assignees for task {TaskId}: {Message}", taskId, message);
        }
        catch (Exception ex)
        {
    ...
    public async Task BroadcastProjectUpdateAsync(int projectId, string updateType, object updateData)
    {
        ...
        {
            // Simplified implementation - would use SignalR in real implementation
            _logger.LogInformation("Project update broadcasted for project {ProjectId}: {UpdateType}", projectId, updateType);
        }
        catch (Exception ex)
        {
